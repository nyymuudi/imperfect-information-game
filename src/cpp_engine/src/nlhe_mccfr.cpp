...
bool NLHEMCCFREngine::load_model(const std::string& path) { return model_.load(path); }
bool NLHEMCCFREngine::load_strategy_model(const std::string& path) { return strategy_model_.load(path); }

std::vector<float> NLHEMCCFREngine::uniform_strategy(int n) {
    return std::vector<float>(n, 1.0f/n);
}

// ── Model strategy: direct 4-action inference (no mapping needed) ─────────────
    ...
    const std::vector<NLHEAction>& actions)
{
    ...
    // C++ uses 4-action enum (same as Python slots), no remapping needed
    auto input = NLHEStateEncoder::encode_tensor(state, player);
    auto probs = model_.forward_tensor(input, config_.max_actions);  // [4]

    std::vector<float> result;
    result.reserve(actions.size());
    for(auto a : actions) {
        int slot = static_cast<int>(a);  // 0=FOLD_OR_CHECK, 1=CALL, 2=RAISE, 3=ALL_IN
    ...
    const NLHEStrategyFn& fn)
{
    ...
    if(p!=tp){
        ...
        int idx=d(rng_);
        return traverse_cb(NLHEGame::apply_action(state,legal[idx]),tp,r_tp,r_opp*probs[idx],fn);
    }
    std::vector<float>vals(n);
    for(int a=0;a<n;++a)
        vals[a]=traverse_cb(NLHEGame::apply_action(state,legal[a]),tp,r_tp*probs[a],r_opp,fn);
    float nv=0.0f;
...
float NLHEMCCFREngine::traverse_model(const NLHEState& state,int tp,float r_tp,float r_opp){
    if(state.terminal) return (tp==0)?state.payoff_p0:-state.payoff_p0;
    const int p=state.current_player;
    const auto legal=NLHEGame::legal_actions(state);
    const int  n=(int)legal.size();
    ...
    if(p!=tp){
        ...
        std::discrete_distribution<int>d(probs.begin(),probs.end());
        int idx=d(rng_);
        return traverse_model(NLHEGame::apply_action(state,legal[idx]),tp,r_tp,r_opp*probs[idx]);
    }
    std::vector<float>vals(n);
    for(int a=0;a<n;++a)
        vals[a]=traverse_model(NLHEGame::apply_action(state,legal[a]),tp,r_tp*probs[a],r_opp);
    float nv=0.0f;
    for(int a=0;a<n;++a) nv+=probs[a]*vals[a];
    const std::string iset=NLHEGame::info_set_key(state,p);
    for(int a=0;a<n;++a){
...
MCCFREngine::BufferExport NLHEMCCFREngine::export_regret_buffer() const {
    MCCFREngine::BufferExport e; const size_t n=regret_buf_.size();
    e.info_sets.reserve(n);e.actions.reserve(n);e.values.reserve(n);e.iterations.reserve(n);
    const auto*d=regret_buf_.data();
    for(size_t i=0;i<n;++i){e.info_sets.push_back(d[i].info_set);e.actions.push_back(d[i].action);e.values.push_back(d[i].regret);e.iterations.push_back(d[i].iteration);}
    return e;
}

MCCFREngine::BufferExport NLHEMCCFREngine::export_strategy_buffer() const {
    MCCFREngine::BufferExport e; const size_t n=strategy_buf_.size();
    e.info_sets.reserve(n);e.actions.reserve(n);e.values.reserve(n);e.iterations.reserve(n);
    const auto*d=strategy_buf_.data();
    for(size_t i=0;i<n;++i){e.info_sets.push_back(d[i].info_set);e.actions.push_back(d[i].action);e.values.push_back(d[i].probability);e.iterations.push_back(d[i].iteration);}
    return e;
}
